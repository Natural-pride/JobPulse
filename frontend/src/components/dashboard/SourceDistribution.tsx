  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      <div className="flex items-baseline justify-between mb-4">
        <h2 className="text-sm font-medium text-slate-200">来源分布</h2>
        <span className="text-xs text-slate-500">{total} 条</span>
      </div>
      {buckets.length === 0 ? (
        <div className="text-xs text-slate-500 py-6 text-center">暂无来源数据</div>
      ) : (
        <div className="flex flex-col gap-2.5">
          {buckets.map((b) => (
            <div key={b.source} className="flex items-center gap-3">
              <div className="w-20 shrink-0 text-xs text-slate-300 truncate" title={b.source}>
                {b.source}
              </div>
